...
// 合并重复课程
// function coursesMergeSame(courses) {
//   let courseMap = {};
//   for (const course of courses) {
//     for (const week of course.weeks) { // 拆成多门单周课
//       const key = JSON.stringify({ // 除了sections需要合并
//         name: course.name,
...
//       });
//       if (!courseMap[key]) {
//         courseMap[key] = new Set(); // 防止section重复
//       }
//       for (const section of course.sections) { // 合并sections
...
//   let coursesMerged = [];
//   for (const [key, set] of Object.entries(courseMap)) { // sections转换回array
//     let course = JSON.parse(key);
//     const sections = Array.from(set).sort((a, b) => a - b);
//     course.sections = sections;
//   }
//   return coursesMerged;
// }
...
//         const key = JSON.stringify({ // weeks day sections相同即为重复或冲突课程
//           weeks: [week],
//           day: course.day,
//           sections: [section],
//         });
...
//           teacher: course.teacher,
//         });
//         if (!courseMap[key]) courseMap[key] = new Set(); // 重复name position teacher会自动合并, 剩下的都是冲突课程
//         courseMap[key].add(value);
//       }
//     }
//   }

//   let courseMap2 = {};
//   for (const [key, valueSet] of Object.entries(courseMap)) {
//     const { day, weeks, sections } = JSON.parse(key);
...
//     let positionMerged = [];
//     let teacherMerged = [];
//     for (const str of valueArray) {
//       const { name, position, teacher } = JSON.parse(str);
...
//       day: day,
//     }
//     const key2 = JSON.stringify(course); // name position teacher weeks day相同即为相同课程
//     course.sections = sections;
//     if (!courseMap2[key2]) { courseMap2[key2] = course }
//     else {
//       courseMap2[key2].sections = Array.from(new Set(courseMap2[key2].sections.concat(course.sections))).sort((a, b) => a - b); // sections合并, 去重, 排序
//     }
//   }
...
// 合并冲突课程
// function coursesMergeConflictsAndDuplicates(courses) {
//   const separator = "&";
//   let coursesOrdered = [];
